...
import type { DoiConfigSnapshot, DoiIntent, DoiTx } from './types.js';
 ...
 *   { action: DoiIntent, config: DoiConfigSnapshot | null }
 * `config` is the row after the write, and null after `reset`.
 */
export async function writeSiteDoiConfigActivity(tx: DoiTx, input: SiteDoiConfigActivityInput) {
  ...
  await tx.activity.create({
    data: {
      ...
      data: { action: input.action, config: input.config },
    },
    select: { id: true },
  });
}
