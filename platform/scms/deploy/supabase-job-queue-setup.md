...
Primary path is the **database enqueue trigger** (`pg_net`). pg_cron is a safety net only — do not rely on it as the main drain mechanism. (Local dev runs the same trigger; its `pg_net` wake targets `host.docker.internal` so it reaches the dev server on the host. Both the `pg_net` and `pg_cron` background workers are bound to the `journals` db locally via `pg_net.database_name` / `cron.database_name` in `docker/postgres/Dockerfile` — without that they attach to the default `postgres` db and silently never drain the `journals` queue. On Supabase the app db is already `postgres`, so the defaults are correct there.)
...
| **Local dev:** jobs stuck, `net.http_request_queue` fills but `net._http_response` stays empty | `pg_net` worker bound to the wrong db. Ensure `pg_net.database_name = 'journals'` is in `docker/postgres/Dockerfile`, then `bun run dx:rebuild`. Verify: `SELECT datname FROM pg_stat_activity WHERE application_name ILIKE '%pg_net%';` should show `journals`. |
