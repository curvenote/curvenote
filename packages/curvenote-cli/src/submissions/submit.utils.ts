...
export async function promptForKey(session: ISession, opts?: { yes?: boolean }): Promise<string> {
  ...
  const { useDefault } = await inquirer.prompt([
    {
      name: 'useDefault',
      message: `Use autogenerated key? (${defaultKey})`,
      type: 'confirm',
      default: true,
    },
  ]);
  ...
  const { customKey } = await inquirer.prompt([
    {
      name: 'customKey',
      type: 'input',
      message: 'Enter the unique key for your work?',
      validate: async (key: string) => {
        ...
        try {
          const { exists } = await getFromJournals(session, `keys/works/${key}`);
