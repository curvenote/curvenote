...
import {
  ...
  type PreviewFigure,
  type DocumentPreviewItem as ClientDocumentPreviewItem,
...
import { downscaleToWebp, isRenderableFigureMime } from './imagePipeline.server.js';
import {
  emptyPreviewAst,
  truncateAstToFirstPage,
  type PreviewAstData,
} from './previewAstUtils.server.js';
import { resolvePreviewImagePresence } from './previewImagePresence.js';

export { resolvePreviewImagePresence } from './previewImagePresence.js';
...
/** Server preview item; structurally assignable to the client {@link ClientDocumentPreviewItem}. */
export interface DocumentPreviewItem extends Omit<ClientDocumentPreviewItem, 'ast'> {
  ast: PreviewAstData;
}
...
async function parseOfficeTextOnly(
  ...
  const { parseOfficeFromBuffer } = await import('./parseOfficeFromBuffer.server.js');
...
async function generatePhaseATextAst(
  ...
  if (isPdfFile(path, file)) {
    const { parsePdfFirstPagePreview, isPdfFastPathTextSufficient } =
      await import('./pdfFirstPagePreview.server.js');
...
async function extractFigureAttachmentsFromBuffer(
  ...
  if (isPdfFile(path, file)) {
    const { extractPdfFigureAttachments } = await import('./pdfFigureExtraction.server.js');
...
// Re-export AST helpers used by extract and tests.
export {
