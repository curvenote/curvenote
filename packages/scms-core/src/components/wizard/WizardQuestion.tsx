...
/** Position of the radio indicator (circle) within each option. */
...
interface WizardQuestionProps<T = string | boolean> {
  ...
  /** When true, vertical/radio_vertical options stay within container (w-full). Default false. */
  verticalConstrainWidth?: boolean;
  /** Position of the radio indicator. Default 'right'. */
...
export function WizardQuestion<T = string | boolean>({
  ...
  iconMap = {},
  specialRenderers = {},
  verticalConstrainWidth = false,
  radioPosition = 'right',
}: WizardQuestionProps<T>) {
  const radioLeft = radioPosition === 'left';
    ...
    if (option.subLabel) {
      label = (
        <div
          className={cn(
            'flex flex-col',
            radioLeft ? 'items-start text-left' : 'items-center text-center',
          )}
        >
      ...
      label = (
        <div className={cn('flex flex-col', radioLeft ? 'items-start text-left' : 'items-center')}>
          <div>{first}</div>
    ...
    return (
      <label
        ...
        className={cn(
          ...
          {
            // Vertical: full width when constrained, else fixed width
            'sm:w-full':
              (question.type === 'vertical' || question.type === 'radio_vertical') &&
              verticalConstrainWidth,
            'sm:w-[522px]':
              (question.type === 'vertical' || question.type === 'radio_vertical') &&
              !verticalConstrainWidth,
      ...
      >
        <RadioGroupItem
          ...
          disabled={disabled}
        />
        {/* Custom radio indicator (position from radioPosition prop) */}
        <span
          className={cn(
        ...
        >
          <div
            className={cn(
              'flex flex-col gap-5 justify-center px-6 sm:flex-col sm:gap-0 grow',
              radioLeft ? 'items-start' : 'items-center',
              {
                'min-h-[120px]':
                  question.type !== 'vertical' &&
                  question.type !== 'radio_vertical' &&
          ...
          >
            ...
            <div
              className={cn(
                'w-full text-md gap-[2px]',
                radioLeft ? 'text-left' : 'text-center sm:w-fit',
                {
    ...
    return (
      <div className={cn('space-y-4', containerClassName)}>
        ...
        <RadioGroup
          ...
          className={cn(
            'grid grid-cols-1 gap-2 p-1 w-full',
            verticalConstrainWidth ? 'max-w-full' : 'max-w-[522px]',
          )}
