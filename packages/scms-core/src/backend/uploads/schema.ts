 ...
 * - maxSize: Maximum per-file size in bytes (e.g., 10MB = 10 * 1024 * 1024)
 * - maxTotalSize: Maximum combined size in bytes for all files in this slot (existing + new batch)
 ...
 * If maxSize is undefined, there is no per-file size limit.
 * If maxTotalSize is undefined, there is no combined slot size limit.
 */
