...
export type SiteDTO = SiteConfig & {
  id: string;
  url?: string;
  /** Tag catalog for the site. Not the version tags on works. */
  tags: TagRefDTO[];
  links: {
    self: string;
    html?: string;
...
/**
 * A tag as exposed on public payloads: the site tag catalog and the tags on
 * published content. `name` is unique per site and URL-safe, so it is the key
 * external consumers filter on; the catalog `id` is an internal write key and
