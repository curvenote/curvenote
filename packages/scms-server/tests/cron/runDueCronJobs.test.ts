...
describe('runDueCronJobs execution', () => {
  ...
  it('runs independent due jobs concurrently and aggregates succeeded/failed correctly', async () => {
    const jobA = jobRow({ id: 'cron-a', name: 'job-a', job_type: 'CHECK_A' });
    const jobB = jobRow({ id: 'cron-b', name: 'job-b', job_type: 'CHECK_B' });
    mockTxSelectQueryRaw
      .mockResolvedValueOnce([jobA, jobB])
      .mockResolvedValueOnce([jobA, jobB]);
    mockEnqueueAndDispatchJob.mockImplementation(async (args: { job_type: string }) => {
      if (args.job_type === 'CHECK_A') {
        return { job_id: 'dispatched-a', status: 'DISPATCHED' };
      }
      if (args.job_type === 'CHECK_B') {
        throw new Error('dispatch failed for job-b');
      }
      throw new Error(`unexpected job_type: ${args.job_type}`);
    });
    ...
    expect(mockCronJobUpdate).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { id: 'cron-a' },
        data: expect.objectContaining({ last_status: 'SUCCESS' }),
      }),
    );
    expect(mockCronJobUpdate).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        data: expect.objectContaining({
          last_status: 'FAILED',
          last_error: 'dispatch failed for job-b',
        }),
      }),
    );
  });
