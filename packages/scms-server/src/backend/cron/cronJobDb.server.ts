...
import { cronEndpointScope } from './scopes.js';
...
function resolveWriteTargetScope({
  targetType,
  targetAuth,
  targetUrl,
  httpMethod,
  ...
  targetType: CronJobTargetType;
  targetAuth: CronJobTargetAuth;
  targetUrl: string | null | undefined;
  httpMethod: string | null | undefined;
  targetScope: string | null | undefined;
}): string | null {
  if (targetType !== CronJobTargetType.HTTP || targetAuth !== CronJobTargetAuth.HANDSHAKE) {
  ...
  if (targetUrl) {
    return cronEndpointScope(httpMethod ?? 'POST', new URL(targetUrl).pathname);
  }
...
export async function dbCreateCronJob(id: string, data: CronJobInput): Promise<CronJob> {
  ...
  const targetAuth = data.target_auth ?? CronJobTargetAuth.HANDSHAKE;
  const targetScope = resolveWriteTargetScope({
...
export async function dbUpdateCronJob(id: string, data: Partial<CronJobInput>): Promise<CronJob> {
  ...
  const targetFieldsChanged =
    ...
    data.http_method !== undefined ||
    data.target_auth !== undefined ||
    data.target_scope !== undefined;
  const targetScope = targetFieldsChanged
    ? resolveWriteTargetScope({
        targetType,
        targetAuth,
        targetUrl,
        httpMethod,
        targetScope: data.target_scope === undefined ? existing.target_scope : data.target_scope,
      })
    : undefined;
  ...
  return prisma.cronJob.update({
    ...
    data: {
      ...
      target_scope: targetScope === existing.target_scope ? undefined : targetScope,
