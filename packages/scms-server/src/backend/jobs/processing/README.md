...
This is separate from **internal job dispatch**, which uses Supabase pgmq (`enqueueAndDispatchJob` → `processJobMessage`). Processing here targets topic-specific pipelines (checks, converter, etc.).
