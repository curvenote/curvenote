...
| ZodError on `redirect_uris` / `client_id` at startup | Not a login attempt — the OAuth client validates metadata when the app boots. Use a **public HTTPS** origin for `clientId`, `redirectUrl`, and `jwksUri` (your tunnel). `http://localhost` in redirects is invalid (use `127.0.0.1` only for `http` loopback, or use HTTPS). `https://localhost` is invalid (loopback cannot use `https` in this schema). |
